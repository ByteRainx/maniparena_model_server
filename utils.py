...
def _decode_msgpack_numpy_object(d: dict) -> Any:
    ...
    # Keys may be str or bytes depending on msgpack raw mode
...
def _extract_instruction(obs: Dict[str, Any]) -> str:
    ...
    Handles:
    - Key variants: ``instruction`` / ``INSTRUCTION`` / ``prompt``
    - Value types: plain str, bytes, np.ndarray, and the msgpack_numpy
      serialized dict that appears when ``np.object_`` arrays are not
      properly deserialized.
    """
    # Try multiple key names (Desktop uses lowercase, CX001 uses UPPERCASE)
    raw = None
    ...
    # Already a numpy array — extract first element
    if isinstance(raw, np.ndarray):
        if raw.size > 0:
            return str(raw.flat[0])
        return ""

    # msgpack_numpy serialized dict (np.object_ array that wasn't deserialized)
